...
function toggleMenu(force?: boolean) {
  ...
  el.querySelector<HTMLButtonElement>("[data-how]")!.onclick = () => {
    toggleMenu(false); showFullGuide();
  };
...
function showFirstHint(force = false) {
  ...
  el.classList.remove("hidden");
  // สี่บรรทัด ไม่ใช่เก้าข้อ — หน้าแรกที่ต้องเลื่อนอ่านคือหน้าที่ไม่มีใครอ่าน
  // ของที่เหลือย้ายไปอยู่ในเมนู "ท่าทั้งหมด" ให้คนที่อยากรู้ไปเปิดเอง
  el.innerHTML = `<b>ท่านคือเทพเจ้าของเกาะนี้</b>
    <ol>
      <li><b>เป้าหมาย</b> — ทำให้มีผู้ศรัทธา ${goalBelievers()} คน
        ตอนนี้มีหมู่บ้านเดียว สิบคน</li>
      <li><b>เลื่อนแผนที่</b> — กดค้างบนพื้นแล้วลาก แผ่นดินจะตามมือไป</li>
      <li><b>ปาฏิหาริย์</b> — เลือกจากแถบขวา แล้วแตะลงบนเกาะ
        เริ่มจาก<b>ฝน</b>ตรงที่ดินแห้ง</li>
      <li><b>สัตว์ของท่าน</b> — ลากนิ้วบนตัวมันคือลูบ ปัดลงเร็วคือตี
        มันจะลังเลก่อนลงมือทุกครั้ง ห้ามตอนนั้นได้ผลกว่าดุทีหลัง</li>
    </ol>
    <div class="sub">หนึ่งวันบนเกาะยาว 6 นาทีจริง กดปุ่ม 1× มุมขวาบนเพื่อเร่ง
      · ท่าทั้งหมดอยู่ในเมนู</div>
...
/** ท่าทั้งหมด — เปิดจากเมนูเมื่ออยากรู้ ไม่ใช่ยัดใส่หน้าแรก */
function showFullGuide() {
  const el = document.getElementById("firsthint")!;
  el.classList.remove("hidden");
  el.innerHTML = `<b>ท่าทั้งหมด</b>
    <ol>
      <li><b>กล้อง</b> — นิ้วเดียวลาก = เลื่อนแผนที่ · สองนิ้ว = หมุนกับซูม
        · บนแมคกด Shift ค้างหรือปุ่มขวาเพื่อหมุน · ล้อเมาส์ซูม
        · แตะสองครั้งที่เดิม = ร่อนลงไปดูใกล้ๆ</li>
      <li><b>มือของท่าน</b> — แตะค้างบนของเพื่อหยิบขึ้นมา แล้วปัดเพื่อขว้าง
        ก้อนหิน ต้นไม้ อาหาร และผู้คน ยกได้หมด
        · คนที่ท่านอุ้มแล้ววางคืนอย่างปลอดภัยจะกลายเป็นนักบวช</li>
      <li><b>คาถาคุยกันได้</b> — ฝนก่อนพร = ดินอุดมกว่า · สายฟ้าลงพื้นเปียก = ไม่เผาแต่ไปไกลกว่า
        · ป่าศักดิ์สิทธิ์บนดินเสีย = ดูดพิษออกไปด้วย</li>
      <li><b>ตีสัตว์มากเกินไปจะไม่ได้สัตว์ที่เชื่อฟัง</b> — จะได้สัตว์ที่ทำตัวเรียบร้อย
        ตอนท่านมอง แล้วไปทำอีกอย่างตอนท่านหันหลัง ดูความกลัวได้ในแผงสัตว์</li>
      <li><b>สอนท่าต่อเนื่องได้</b> — ชมคู่เดิมสองครั้ง เช่นช่วยหมู่บ้านแล้วบูชา
        มันจะทำสองอย่างนั้นต่อกันเอง</li>
    ...
    <button id="hintOk">ปิด</button>`;
  document.getElementById("hintOk")!.onclick = () => el.classList.add("hidden");
}
